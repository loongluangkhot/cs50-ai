...
def load_data(filename):
    ...
    with open(filename, newline='') as csvfile:
        ...
        for row in reader:
            ...
            page_val = float(row[8])
            special_day = float(row[9])
            month = 5 if row[10] == 'June' else datetime.strptime(row[10], '%b').month - 1
            os = int(row[11])
            browser = int(row[12])
            region = int(row[13])
            traffic = int(row[14])
...
def evaluate(labels, predictions):
    ...
    for (actual, predict) in zip(labels, predictions):
        if actual == 0:
            ...
            if actual == predict:
                pos_identified += 1
    sensitivity = pos_identified / pos_actual
    specificity = neg_identified / neg_actual
